...
#include <vector>
using namespace std;
int minimizeCostHelper(int n, int k, vector<int> &height, vector<int> &dp)
{
    int maxim = INT_MAX;
    if (n == 0)
        return 0;
    ...
    for (int i = 1; i <= k; i++)
    {
        if (n - i >= 0)
        {
            int ks = minimizeCostHelper(n - i, k, height, dp) +
                     abs(height[n] - height[n - i]);
...
{
    vector<int> dp(n, 0);
    minimizeCostHelper(n - 1, k, height, dp);
    return dp[n - 1];
}
